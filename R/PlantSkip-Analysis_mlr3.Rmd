...
for(i in 1:length(task_names)){
  ...
  dat <- dat_list[[i]]
  dat.1 <- dplyr::select(dat, -dob, -bui, -dc, -isi)
  dat.2 <- dplyr::select(dat, -dob, -bui, -dmc, -isi)
  dat.3 <- dplyr::select(dat, -dob, -bui, -isi)
  dat.4 <- dplyr::select(dat, -dob, -bui, -dc, -fwi)
  dat.5 <- dplyr::select(dat, -dob, -bui, -dmc, -fwi)
  dat.6 <- dplyr::select(dat, -dob, -bui, -fwi)
  ...
  #--- Step 3: Benchmark - assess the multiple tasks
  ...
  #--- Step 4: Feature selection & task benchmarking (using nested resampling)

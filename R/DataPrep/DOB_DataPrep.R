...
for (i in 1:length(DOB_list)){
  ...
  DOB_rounded[[i]] <- round(DOB[[i]], digits = 0)
  # Write rasters
  writeRaster(DOB_rounded[[i]], paste0(SpatialFilesPath, "./Inputs/DOB/DOBrounded/", "DOBrounded_", file.name[[i]]), format = "GTiff", overwrite = TRUE)
}

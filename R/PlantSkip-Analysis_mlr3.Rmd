...
TrainImp_dat[[3]][,FireID := task_names[3]]
TrainImp_dat[[4]][,FireID := task_names[4]]
TrainImp_dat[[5]][,FireID := task_names[5]]
TrainImp_dat[[6]][,FireID := task_names[6]]
...
pd_scores[[1]][,FireID := task_names[1]]
pd_scores[[2]][,FireID := task_names[2]]
pd_scores[[3]][,FireID := task_names[3]]
pd_scores[[4]][,FireID := task_names[4]]
pd_scores[[5]][,FireID := task_names[5]]
pd_scores[[6]][,FireID := task_names[6]]
pd_scores_dt <- rbindlist(pd_scores)
...
terminator = trm("stagnation", iters=20, threshold=0)
...
```{r}
library(mgcv)
mod <- gam(dNBR ~ s(PlantAge, bs="cr"), data = Chutanli)
summary(mod)
